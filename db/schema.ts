...
import { bigint, boolean, customType, index, integer, pgTable, text, timestamp, uniqueIndex } from "drizzle-orm/pg-core";
...
// Índices: Postgres não indexa colunas de FK automaticamente (só a PK e
// constraints unique/exclusion) — companyId/supplierId são o filtro de
// âmbito (company_admin → própria empresa, supplier → próprio fornecedor)
// usado em quase todas as rotas de listagem desta app, por isso ganham
// índice próprio em cada tabela onde existem, aqui e nas tabelas a seguir.
export const users = pgTable(
  ...
  (table) => [index("users_company_id_idx").on(table.companyId), index("users_supplier_id_idx").on(table.supplierId)]
);
...
export const requests = pgTable(
  ...
  (table) => [index("requests_company_id_idx").on(table.companyId), index("requests_owner_user_id_idx").on(table.ownerUserId)]
);
...
export const purchaseOrders = pgTable(
  ...
  (table) => [
    ...
    index("purchase_orders_request_id_idx").on(table.requestId),
  ]
);
...
export const poEvents = pgTable(
  ...
  (table) => [index("po_events_po_id_idx").on(table.poId)]
);
...
export const tenders = pgTable(
  ...
  (table) => [index("tenders_company_id_idx").on(table.companyId)]
);
...
export const tenderInvites = pgTable(
  ...
  (table) => [
    uniqueIndex("tender_invites_tender_supplier_idx").on(table.tenderId, table.supplierId),
    // O índice único acima cobre buscas por tenderId (é a coluna à
    // esquerda), mas não por supplierId sozinho — "a que tenders este
    // fornecedor foi convidado" precisa do seu próprio índice.
    index("tender_invites_supplier_id_idx").on(table.supplierId),
  ]
);
...
export const bids = pgTable(
  ...
  (table) => [
    ...
    index("bids_supplier_id_idx").on(table.supplierId),
  ]
);
...
export const contracts = pgTable(
  ...
  (table) => [index("contracts_company_id_idx").on(table.companyId), index("contracts_supplier_id_idx").on(table.supplierId)]
);
...
export const catalogItems = pgTable(
  ...
  (table) => [index("catalog_items_supplier_id_idx").on(table.supplierId)]
);
...
export const receipts = pgTable(
  ...
  (table) => [index("receipts_company_id_idx").on(table.companyId), index("receipts_supplier_id_idx").on(table.supplierId)]
);
...
export const exceptions = pgTable(
  ...
  (table) => [index("exceptions_company_id_idx").on(table.companyId)]
);
...
export const clientInvoices = pgTable(
  ...
  (table) => [index("client_invoices_company_id_idx").on(table.companyId)]
);
...
export const clientInvoiceLines = pgTable(
  ...
  (table) => [index("client_invoice_lines_client_invoice_id_idx").on(table.clientInvoiceId)]
);
...
export const documents = pgTable(
  ...
  (table) => [index("documents_entity_type_id_idx").on(table.entityType, table.entityId)]
);
...
export const applications = pgTable(
  ...
  (table) => [index("applications_assigned_to_user_id_idx").on(table.assignedToUserId)]
);
...
export const supportTickets = pgTable(
  ...
  (table) => [
    index("support_tickets_company_id_idx").on(table.companyId),
    index("support_tickets_user_id_idx").on(table.userId),
    index("support_tickets_assigned_to_user_id_idx").on(table.assignedToUserId),
  ]
);
